...
public static class MauiProgram
{
	...
	public static MauiApp CreateMauiApp()
	{
		...
		// encrypted device backups and restores onto a different device — so
		// the device token and payload key would reconstitute a working Reach
		// enrolment on a handset the intergroup never enrolled. ThisDeviceOnly
		// has identical unlock semantics, which matters here because a push can
		// arrive on a locked phone, and excludes the item from migration.
		//
		// This is the iOS counterpart of allowBackup="false" on Android, and it
		...
		string environment)
	{
...
#if DEBUG
			// After the configured level, so it wins. appsettings asks for
			// Information, which is right for a responder's phone and wrong for
			// a cable and a terminal: it hides every Log.Debug line, and those
			// are the ones written for exactly this situation.
			.MinimumLevel.Debug()
#endif
		...
		// hits SelfLog with a stack trace and drowns real diagnostics. So it stays
		// desktop only, and Android gets a sink of its own below rather than a
		// workaround for this one.
#if WINDOWS || MACCATALYST
		cfg = cfg.WriteTo.Console();
#endif

		// Live output on a handset, which there was none of. The comment above
		// used to end "the Debug sink already surfaces logs to the IDE" — true,
		// and worth nothing to an app launched by adb, which is every deployment
		// /kick makes. No listener, so the only record was the file, readable
		// after the fact and only by somebody who knew to ask for it.
		//
		//     adb -s <serial> logcat -s Hand:V
		//
		// Verified absent before this went in rather than assumed: Hand was
		// running on a Pixel and its process log held not one Serilog line. See
		// LogcatSink, including why it is not the console sink.
#if ANDROID
		cfg = cfg.WriteTo.Sink(new LogcatSink());
#endif
#else
	...
	private static HttpClient CreateHttpClient()
	{
		...
		// Wrapped round the platform handler rather than set on the client:
		// part of what the header reports is the server this handset is
		// pointed at, and a responder can change that from the settings
		// screen without restarting the app. See UserAgentHandler.
	...
	private static HttpClient CreateBetterStackHttpClient()
	{
		...
		// Better Stack gets the same introduction as Reach does. It is not
		// behind the bot protection this was written for, but a log shipper
		// that names itself is worth having in its own right.
		return new HttpClient(new UserAgentHandler(AppUserAgent.Current, handler), disposeHandler: true)
		{
